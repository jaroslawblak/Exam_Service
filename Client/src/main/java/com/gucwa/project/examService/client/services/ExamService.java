...
public class ExamService {
    ...
    public Exam getExam() {
        return restTemplate.getForObject("http://localhost:8080/exams", Exam.class);
    }

    // teraz robimy post, wiec ten co odbieram i ten co wysylam musi byc
    public Exam veryfyExam(Exam exam) {
        return restTemplate.postForObject("http://localhost:8080/exams", exam, Exam.class);
    }

    public Boolean veryfyUser(String password) {
        return restTemplate.postForObject("http://localhost:8080/admins/password", password, Boolean.TYPE);
    }
}
